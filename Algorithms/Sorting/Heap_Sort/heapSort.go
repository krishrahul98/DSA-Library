...
func newMinHeap(arr []int) *minheap {
    minheap := &minheap{arr: arr}
    return minheap
}

func (m *minheap) rightchildIndex(index int) int { return 2*index + 2 }
func (m *minheap) leftchildIndex(index int) int { return 2*index + 1 }

func (m *minheap) leaf(index int, size int) bool {
    if index <= size && index >= (size/2) { return true }
    return false
}

func (m *minheap) swap(first int, second int) {
    tmp := m.arr[first]
...
func main() {
    input := []int{68, 5, 63, 7, 542, 18, 7 ,2, -1}
    minHeap := newMinHeap(input)
